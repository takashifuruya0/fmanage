...
def _create_opportunity(i, res, u, type_opp, is_linked_to_client=True):
    try:
        # category
        if Category.objects.filter(name=i['category']).count() == 1:
            category = Category.objects.get(name=i['category'])
...
def test5(is_direct=True, is_proposal=True, is_else=True):
    url = "https://script.google.com/macros/s/AKfycbz_SWwkiNpdnWHUOx_WF1i1kRZYu0I-gKo5N_-1wuzHH1yjm2tS/exec"
    r = requests.get(url)
    data = r.json()['data']
    di = {
    ...
    if is_direct:
        for i in di['直接受注']:
            ...
            try:
                ...
                continue
            res.pop("client_url")
            result = _create_opportunity(i, res, u, "直接受注")
            if result:
    ...
    if is_proposal:
        for i in di['提案受注']:
            ...
            res.pop("opportunity_url")
            res.pop("client_url")
            result = _create_opportunity(i, res, u, "提案受注")
            if result:
    ...
    if is_else:
        for i in di['else']:
            ...
            result = _create_opportunity(i, res, u, "直接受注", is_linked_to_client=False)
            if result:
